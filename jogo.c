...
// STRUCTS
typedef struct {
    Rectangle area;          // Posição e tamanho do animal na tela (x, y, largura, altura). Usado para desenho e detecção de colisão.
    ...
    bool atordoada;          // Indica se o animal está no estado de "atordoado" após ser acertado.
...
bool DesenharBotao(Rectangle area, const char* texto, Vector2 mousePos) {
    ...
    DrawRectangleRec(area, cor); // Desenha o retângulo do botão com a cor definida.
    // Desenha o texto do botão, centralizado horizontalmente e ajustado verticalmente.
    DrawText(texto, area.x + (area.width - MeasureText(texto, 30)) / 2, area.y + 15, 30, WHITE);
...
void InicializarJogo(Dificuldade dificuldade) {
    ...
    for (int i = 0; i < totalCapivaras; i++) {
        ...
        capivaras[i].area = (Rectangle){ (float)(startX + i * (larguraCapivara + espacamentoCapivara)), (float)yPos, (float)larguraCapivara, (float)alturaCapivara };
        // Inicializa todos os outros campos da estrutura Capivara para seus valores padrão.
        // Isso garante que cada capivara comece invisível, não acertada, não atordoada e do tipo NORMAL.
        capivaras[i] = (CAPIVARA){capivaras[i].area, false, 0.0, 0.0, false, 0.0, false, NORMAL};
    }
}
    ...
    for (int i = 0; i < totalCapivaras; i++) {
        ...
        if (capivaras[i].atordoada) {
            ...
            if (capivaras[i].tempoAcertada >= 0.5) {      // Após 0.5 segundos (tempo da animação de atordoado).
                // Reseta o estado da capivara: invisível, não acertada, não atordoada, e volta ao tipo NORMAL para a próxima aparição.
                capivaras[i] = (CAPIVARA){capivaras[i].area, false, 0.0, 0.0, false, 0.0, false, NORMAL};
            }
            continue; // Pula para a próxima capivara no loop, pois esta já foi tratada neste frame.
        }

        // Lógica para capivaras que estão visíveis (mas não atordoadas) ou para fazer novas aparecerem.
        ...
        if (capivaras[i].visivel && !capivaras[i].hit && IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && CheckCollisionPointRec(GetMousePosition(), capivaras[i].area)) {
            capivaras[i].hit = true;        // Marca a capivara como acertada.
            capivaras[i].atordoada = true;  // Inicia o estado de "atordoada"
...
void DesenharJogo() {
    ...
    for (int i = 0; i < totalCapivaras; i++) {
        // Se a capivara não está visível e não está atordoada, não há necessidade de desenhá-la.
        if (!capivaras[i].visivel && !capivaras[i].atordoada)
        ...
        Rectangle destRec = capivaras[i].area; // O retângulo de destino onde a textura será desenhada.
        ...
        if (capivaras[i].tipo == CUTIA) {
            ...
            // Centraliza a cutia dentro do espaço original do slot (capivaras[i].area).
            destRec.x += (capivaras[i].area.width - destRec.width) / 2;
            destRec.y += (capivaras[i].area.height - destRec.height) / 2;
        }

        // Desenha a textura apropriada: atordoada se atordoada, ou a textura normal/dourada/cutia se visível.
        if (capivaras[i].atordoada) {
            // Se estiver atordoada, sempre desenha a textura de atordoado.
            ...
            // Se estiver visível e não atordoada, desenha a textura do tipo de animal.
